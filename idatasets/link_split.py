...
def undirected_label2directed_label(adj, edge_pairs, task, directed_graph=True, signed_directed=False):
    ...
    Arg types:
        edge_pair(List[Tuple]): The edge list for the link dataset querying. Each element in the list is an edge tuple.
        task:   three_class_digraph (three-class link prediction); 
                direction (direction prediction); 
                existence (existence prediction); 
                four_class_signed_digraph (directed sign prediction); 
                five_class_signed_digraph (directed sign and existence prediction) 
                sign (sign prediction); 
    Return types:
        new_edge_pairs(List):   A list of edges.
        labels(List): The labels for new_edge_pairs. 
        label_weight(List): The weight list of the query edges. The weight is zero if the directed edge doesn't exist in both directions.
    ...
    if signed_directed:
        directed_pos = (np.array(adj[edge_pairs[:, 0], edge_pairs[:, 1]]).flatten() > 0).tolist()
        directed_neg = (np.array(adj[edge_pairs[:, 0], edge_pairs[:, 1]]).flatten() < 0).tolist()
        inversed_pos = (np.array(adj[edge_pairs[:, 1], edge_pairs[:, 0]]).flatten() > 0).tolist()
        inversed_neg = (np.array(adj[edge_pairs[:, 1], edge_pairs[:, 0]]).flatten() < 0).tolist()
        ...
        negative = np.array(list(set(edge_pairs) - set(directed_pos) - set(inversed_pos) - set(directed_neg) - set(inversed_neg)))
        ...
        labels = np.vstack([np.zeros((len(directed_pos), 1), dtype=np.int32), np.ones((len(directed_neg), 1), dtype=np.int32)])

        labels = np.vstack([labels, 2 * np.ones((len(directed_pos), 1), dtype=np.int32), 3 * np.ones((len(directed_neg), 1), dtype=np.int32)])

        labels = np.vstack([labels, 4*np.ones((len(negative), 1), dtype=np.int32)])

        label_weight = np.vstack([np.array(adj[directed_pos[:, 0], directed_pos[:, 1]]).flatten()[:, None], np.array(adj[directed_neg[:, 0], directed_neg[:, 1]]).flatten()[:, None]])
...
def link_class_split(edge_index, A, cache_edge_split, edge_split_id, size=None, splits=10, prob_test=0.15,
                     prob_val=0.05, task='direction', seed=0, maintain_connect=True, ratio=1.0):
    r"""Get train/val/test dataset for the link prediction task. 
    Arg types:
        size:   The size of the input graph. If none, the graph size is the maximum index of nodes plus 1 (Default: None).
        task:   The evaluation task: 
                three_class_digraph (three-class link prediction); 
                direction (direction prediction); 
                existence (existence prediction); 
                sign (sign prediction); 
                four_class_signed_digraph (directed sign prediction); 
                five_class_signed_digraph (directed sign and existence prediction) 
                (Default: 'direction')
        seed:   The random seed for positve edge selection (Default: 0). Negative edges are selected by pytorch geometric negative_sampling.
        maintain_connect:   If maintaining connectivity when removing edges for validation and testing. 
                            The connectivity is maintained by obtaining edges in the minimum spanning tree/forest first. 
                            These edges will not be removed for validation and testing 
                            (Default: True). 
        ratio:  The maximum ratio of edges used for dataset generation. (Default: 1.0)
    Return types:
        A dict include training/validation/testing splits of edges and labels. 
            idatasets[i]['graph'] (torch.LongTensor): the observed edge list after removing edges for validation and testing.
            idatasets[i]['train'/'val'/'testing']['edges'] (List): the edge list for training/validation/testing.
            idatasets[i]['train'/'val'/'testing']['label'] (List): the labels of edges:
                The undirected edges in the directed input graph are removed to avoid ambiguity.
                    1. If task == "existence":  0 (the directed edge exists in the graph), 
                                                1 (the edge doesn't exist). 
                    2. If task == "direction":  0 (the directed edge exists in the graph), 
                                                1 (the edge of the reversed direction exists). 
                    3. If task == "three_class_digraph":    0 (the directed edge exists in the graph), 
                                                            1 (the edge of the reversed direction exists), 
                                                            2 (the edge doesn't exist in both directions). 
                    4. If task == "four_class_signed_digraph":  0 (the positive directed edge exists in the graph), 
                                                                1 (the negative directed edge exists in the graph), 
                                                                2 (the positive edge of the reversed direction exists),
                                                                3 (the edge of the reversed direction exists). 
                    5. If task == "five_class_signed_digraph":  0 (the positive directed edge exists in the graph)
                                                                1 (the negative directed edge exists in the graph), 
                                                                2 (the positive edge of the reversed direction exists),
                                                                3 (the edge of the reversed direction exists), 
                                                                4 (the edge doesn't exist in both directions). 
                    6. If task == "sign":   0 (negative edge), 
                                            1 (positive edge). 
                    This is the link sign prediction task for signed networks.
    """
    ...
    try:
        ...
        if maintain_connect:
            ...
            #     A_undirected, nodelist=nx.Graph, weight='weight')
        ...
        for ind in range(splits):
            ...
            observed_weight[direct, 0] = np.array(A[ids_train[direct, 0], ids_train[direct, 1]]).flatten()
            ...
            if len(undirected_train) > 0:
                ...
                observed_edges = np.vstack((observed_edges, undirected_train))
                observed_weight = np.vstack((observed_weight, np.array(A[undirected_train[:, 0], undirected_train[:, 1]]).flatten()[:, None]))

            assert(len(edge_index.T) >= len(observed_edges)), 'The original edge number is {} while the observed graph has {} edges!'.format(len(edge_index.T), len(observed_edges))

            datasets[ind] = {}
            datasets[ind]['graph'] = torch.from_numpy(observed_edges.T).long()
            datasets[ind]['weights'] = torch.from_numpy(observed_weight.flatten()).float()

            datasets[ind]['train'] = {}
            datasets[ind]['train']['edges'] = torch.from_numpy(ids_train).long()
            datasets[ind]['train']['label'] = torch.from_numpy(labels_train).long()

            datasets[ind]['val'] = {}
            datasets[ind]['val']['edges'] = torch.from_numpy(ids_val).long()
            datasets[ind]['val']['label'] = torch.from_numpy(labels_val).long()

            datasets[ind]['test'] = {}
            datasets[ind]['test']['edges'] = torch.from_numpy(ids_test).long()
            datasets[ind]['test']['label'] = torch.from_numpy(labels_test).long()
