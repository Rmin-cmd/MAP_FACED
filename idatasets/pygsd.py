...
from idatasets.node_split import node_class_split
from idatasets.utils import pkl_read_file, download_to, remove_self_loops, coomatrix_to_torch_tensor, edge_homophily, \
    node_homophily, linkx_homophily, set_spectral_adjacency_reg_features
...
class PyGSDDataset(NodeDataset):
    ...
    Notably, in the version of the directed networks, we select the original Chameleon, Squirrel idatasets from ICLR'20 GEOM-GCN: GEOMETRIC GRAPH CONVOLUTIONAL NETWORKS, https://arxiv.org/pdf/2002.05287.pdf and NeurIPS'22 Revisiting Heterophily For Graph Neural Networks, https://arxiv.org/pdf/2210.07606.pdf.
    ...
    def __init__(self, args, name, root, k, node_split, node_split_id=0, edge_split="direction", edge_split_id=0):
        ...
        if self.name in ['wikics', 'roman-empire', 'amazon-rating', 'tolokers', "squirrelfilterdir", "chameleonfilterdir", 
        ...
        if args.heterophily and self.name not in ("wikitalk", "slashdot", "epinions"):
    ...
    def raw_file_paths(self):
        dataset_name = {
            ...
            'amazon-rating':'amazon_ratings.npz',
            'wikitalk': 'wikitalk.npz',
        }
        if self.name in ("coraml", "citeseerdir", "wikitalk", "slashdot", "epinions", "amazon-computers", "tolokers", 
                         "roman-empire", "amazon-rating", "squirrelfilterdir", "chameleonfilterdir", "actor", "texas", "cornell", "wisconsin"):
    ...
    def process(self):
        ...
        elif self.name in ("amazon-computers"):
            with np.load(self.raw_file_paths[0], allow_pickle=True) as loader:
                loader = dict(loader)
        ...
        elif self.name in ("tolokers", "roman-empire", "amazon-rating", "squirrelfilterdir", "chameleonfilterdir", "actor", "texas", "cornell", "wisconsin"):
            with np.load(self.raw_file_paths[0], allow_pickle=True) as loader:
                loader = dict(loader)
